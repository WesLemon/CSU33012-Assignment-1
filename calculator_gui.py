...
class GUI:
    ...
    def __init__(self):
        ...
        btn_equals = tk.Button(self.root, text="=", command=self.logic, width=11,
                               font=("Arial", 14))
    ...
    def add_to_calculation(self, symbol):
        global toCalculate
        toCalculate += str(symbol)
        self.text_result.delete(1.0, "end")
        self.text_result.insert(1.0, toCalculate)

    def logic(self):
        global toCalculate
        expression = calc.convert_string_to_list(toCalculate)
        if expression is None:
            self.clear_field()
            self.text_result.insert(1.0, 'Error: Invalid String')
            toCalculate = ""
        elif calc.check_validity(expression):
            result = calc.calculate(expression)
            self.text_result.delete(1.0, "end")
            self.text_result.insert(1.0, result)
            toCalculate = str(result)
        else:
            self.clear_field()
            self.text_result.insert(1.0, 'Error: Invalid String')
            toCalculate = ""